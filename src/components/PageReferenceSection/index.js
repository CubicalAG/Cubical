...
const PageReferenceSection = ({ data: sliceData }) => {
  ...
  const data = useStaticQuery(graphql`
    query PrismicSectionQuery {
      references: allPrismicReference(
  ...
  useEffect(() => {
    if (
      ...
      data.references.edges.length > 0
    ) {
      setFilteredData(
        data.references.edges.filter(({ node: reference }) => {
          if (
            reference &&
            reference.data &&
            reference.data.reference_tag &&
  ...
  return (
    ...
    filteredData.length > 0 && (
      <Section
        id={
          sliceData && sliceData.primary && sliceData.primary.section_id
            ...
            ({ node: reference }, index) => (
              ...
              <div className={`${styles.referenceItem} ${styles.visibleItem}`}>
                <Reference
                  ...
                  buttonLink={reference.data.button_link}
                  image={
